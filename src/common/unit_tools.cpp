...
uint32_t Common::getCtuAddr(const Position &pos, const PreCalcValues &pcv) {
...
bool CU::hasSubCUNonZeroAffineMVd(const CodingUnit &cu) {
  ...
  for (const auto &pu : Common::CU::traversePUs(cu)) {
    if ((!pu.mergeFlag) && (!cu.skip)) {
      if (pu.interDir != 2 /* PRED_L1 */) {
...
bool Common::allowLfnstWithMip(const Size &block) {
...
int Common::getNumModesMip(const Size &block) {
...
int Common::getMipSizeId(const Size &block) {
