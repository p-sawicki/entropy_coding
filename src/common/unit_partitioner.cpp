...
Common::PartLevel::PartLevel(const PartSplit _split,
...
Common::SplitSeries Common::Partitioner::getSplitSeries() const {
  ...
  for (const auto &level : m_partStack) {
    if (level.split == CTU_LEVEL)
      ...
      splitSeries += static_cast<SplitSeries>(level.split)
                     << (depth * SPLIT_DMULT);
...
Common::ModeTypeSeries
Common::Partitioner::getModeTypeSeries() const {
...
void Common::Partitioner::setCUData(CodingUnit &cu) {
  cu.depth = currDepth;
...
bool Common::Partitioner::isSepTree(const CodingStructure &cs) {
  return treeType != TREE_D || CS::isDualITree(cs);
}

void Common::QTBTPartitioner::initCtu(const UnitArea &ctuArea,
  ...
  currQgChromaPos =
      ctuArea.chromaFormat != CHROMA_400 ? ctuArea.chromaPos() : Position();
...
void Common::QTBTPartitioner::splitCurrArea(const PartSplit split,
  ...
  switch (split) {
  case CU_QUAD_SPLIT:
    m_partStack.push_back(
        PartLevel(split, PartitionerImpl::getCUSubPartitions(currArea(), cs)));
    m_partStack.back().modeType = modeType;
    break;
  case CU_HORZ_SPLIT:
  case CU_VERT_SPLIT:
    m_partStack.push_back(PartLevel(
        split, PartitionerImpl::getCUSubPartitions(currArea(), cs, split)));
    m_partStack.back().modeType = modeType;
    break;
  case CU_TRIH_SPLIT:
  case CU_TRIV_SPLIT:
  ...
  case TU_MAX_TR_SPLIT:
    m_partStack.push_back(
        PartLevel(split, PartitionerImpl::getMaxTuTiling(currArea(), cs)));
    break;
  ...
  case SBT_HOR_QUAD_POS1_SPLIT:
    m_partStack.push_back(PartLevel(
        split, PartitionerImpl::getSbtTuTiling(currArea(), cs, split)));
    break;
  default:
  ...
  if (split == TU_MAX_TR_SPLIT) {
    currTrDepth++;
  } else if (split >= SBT_VER_HALF_POS0_SPLIT &&
             split <= SBT_HOR_QUAD_POS1_SPLIT) {
    currTrDepth++;
  } else {
    currTrDepth = 0;
  }
  ...
  if (split == CU_HORZ_SPLIT || split == CU_VERT_SPLIT ||
      split == CU_TRIH_SPLIT || split == CU_TRIV_SPLIT) {
    currBtDepth++;
    if (isImplicit)
      currImplicitBtDepth++;
    currMtDepth++;
    ...
    CHECK(currBtDepth > 0,
          "Cannot split a non-square area other than with a binary split");
    CHECK(currMtDepth > 0,
          "Cannot split a non-square area other than with a binary split");
    currMtDepth = 0;
    currBtDepth = 0;
...
void Common::QTBTPartitioner::exitCurrSplit() {
  ...
  if (currSplit == CU_HORZ_SPLIT || currSplit == CU_VERT_SPLIT ||
      currSplit == CU_TRIH_SPLIT || currSplit == CU_TRIV_SPLIT) {
    CHECK(!m_partStack.back().checkdIfImplicit,
          "Didn't check if the current split is implicit");
    CHECK(currBtDepth == 0,
          "BT depth is '0', athough a BT split was performed");
    CHECK(currMtDepth == 0,
          "MT depth is '0', athough a BT split was performed");
    currMtDepth--;
    ...
    if ((currSplit == CU_TRIH_SPLIT || currSplit == CU_TRIV_SPLIT) &&
        currIdx != 1) {
      CHECK(currBtDepth == 0,
            "BT depth is '0', athough a TT split was performed");
      currBtDepth--;
      currSubdiv--;
    }
  } else if (currSplit == TU_MAX_TR_SPLIT) {
    CHECK(currTrDepth == 0,
          "TR depth is '0', although a TU split was performed");
    currTrDepth--;
  } else if (currSplit >= SBT_VER_HALF_POS0_SPLIT &&
             currSplit <= SBT_HOR_QUAD_POS1_SPLIT) {
    CHECK(currTrDepth == 0,
          "TR depth is '0', although a TU split was performed");
    currTrDepth--;
  } else {
    ...
    CHECK(currQtDepth == 0,
          "QT depth is '0', although a QT split was performed");
    currQtDepth--;
    currSubdiv--;
  }
}

bool Common::QTBTPartitioner::nextPart(const CodingStructure &cs,
                                              bool autoPop /*= false*/) {
  ...
  if (currIdx == 1) {
    ...
    m_partStack.back().firstSubPartSplit =
        prevCU ? CU::getSplitAtDepth(*prevCU, currDepth) : CU_DONT_SPLIT;
  }

  if (currIdx < m_partStack.back().parts.size()) {
    if (m_partStack.back().split == CU_TRIH_SPLIT ||
        m_partStack.back().split == CU_TRIV_SPLIT) {
...
void Common::QTBTPartitioner::canSplit(const CodingStructure &cs,
  ...
  const unsigned maxBTD =
      cs.pcv->getMaxBtDepth(*cs.slice, chType) + currImplicitBtDepth;
  ...
  const CompArea *areaC =
      (chType == CHANNEL_TYPE_CHROMA) ? &(currArea().Cb()) : nullptr;
  PartLevel &level = m_partStack.back();

  const PartSplit lastSplit = level.split;
  const PartSplit parlSplit =
      lastSplit == CU_TRIH_SPLIT ? CU_HORZ_SPLIT : CU_VERT_SPLIT;
  ...
  if ((lastSplit == CU_TRIH_SPLIT || lastSplit == CU_TRIV_SPLIT) &&
      currPartIdx() == 1) {
  ...
  if (modeType == MODE_TYPE_INTER && area.width * area.height == 32)
    canBv = canBh = false;
  if (area.height <= 2 * minTtSize || area.height > maxTtSize ||
      area.width > maxTtSize)
    canTh = false;
  if (area.width > MAX_TB_SIZEY || area.height > MAX_TB_SIZEY)
    canTh = false;
  if (areaC && areaC->width * areaC->height <= MIN_DUALTREE_CHROMA_SIZE * 2)
    canTh = false;
  if (area.width <= 2 * minTtSize || area.width > maxTtSize ||
      area.height > maxTtSize)
    canTv = false;
  if (area.width > MAX_TB_SIZEY || area.height > MAX_TB_SIZEY)
    canTv = false;
  if (areaC && (areaC->width * areaC->height <= MIN_DUALTREE_CHROMA_SIZE * 2 ||
...
bool Common::QTBTPartitioner::canSplit(const PartSplit split,
                                              const CodingStructure &cs) {
  const CompArea area = currArea().Y();
...
bool Common::QTBTPartitioner::isSplitImplicit(
    const PartSplit split, const CodingStructure &cs) {
  return split == getImplicitSplit(cs);
}

Common::PartSplit
Common::QTBTPartitioner::getImplicitSplit(const CodingStructure &cs) {
  ...
  if (split == CU_DONT_SPLIT) {
    const bool isBlInPic =
        cs.picture->Y().contains(currArea().Y().bottomLeft());
    const bool isTrInPic = cs.picture->Y().contains(currArea().Y().topRight());

    const CompArea &area = currArea().Y();
    const unsigned maxBtSize = cs.pcv->getMaxBtSize(*cs.slice, chType);
    const bool isBtAllowed =
        area.width <= maxBtSize && area.height <= maxBtSize &&
        currMtDepth <
            (cs.pcv->getMaxBtDepth(*cs.slice, chType) + currImplicitBtDepth);
    const unsigned minQtSize = cs.pcv->getMinQtSize(*cs.slice, chType);
    const bool isQtAllowed =
        area.width > minQtSize && area.height > minQtSize && currBtDepth == 0;
    ...
    if (CS::isDualITree(cs) &&
        (currArea().Y().width > 64 || currArea().Y().height > 64)) {
      split = CU_QUAD_SPLIT;
    }
...
void Common::TUIntraSubPartitioner::splitCurrArea(
    const PartSplit split, const CodingStructure &cs) {
  switch (split) {
  ...
  case TU_1D_VERT_SPLIT: {
    ...
    PartitionerImpl::getTUIntraSubPartitions(m_partStack.back().parts, area, cs,
                                             split);
    break;
  }
  case TU_MAX_TR_SPLIT: // we need this non ISP split because of the maxTrSize
                        // limitation
    m_partStack.push_back(
        PartLevel(split, PartitionerImpl::getMaxTuTiling(currArea(), cs)));
    break;
  ...
  currDepth++;
  currTrDepth++; // we need this to identify the level. since the 1d partitions
                 // are forbidden if the RQT is on, there area no compatibility
                 // issues
...
void Common::TUIntraSubPartitioner::exitCurrSplit() {
  PartSplit currSplit = m_partStack.back().split;
  ...
  CHECK(!(currSplit == TU_1D_HORZ_SPLIT || currSplit == TU_1D_VERT_SPLIT ||
          currSplit == TU_MAX_TR_SPLIT),
        "Unknown 1D partition split type!");
}

bool Common::TUIntraSubPartitioner::nextPart(const CodingStructure &cs,
                                                    bool autoPop /*= false*/) {
  unsigned currIdx = ++m_partStack.back().idx;
...
bool Common::TUIntraSubPartitioner::canSplit(const PartSplit split,
                                                    const CodingStructure &cs) {
...
Common::Partitioning Common::PartitionerImpl::getCUSubPartitions(
  ...
  if (splitType == CU_QUAD_SPLIT) {
    if (!cs.pcv->noChroma2x2) {
      ...
      for (uint32_t i = 0; i < 4; i++) {
        ...
        CHECK(sub[i].lumaSize().height < MIN_TB_SIZEY,
              "the split causes the block to be smaller than the minimal TU "
              "size");
      }
      ...
      bool canSplit = cuArea.lumaSize().width > minCUSize &&
                      cuArea.lumaSize().height > minCUSize;
    ...
    for (uint32_t i = 0; i < 2; i++) {
      ...
      CHECK(sub[i].lumaSize().height < MIN_TB_SIZEY,
            "the cs split causes the block to be smaller than the minimal TU "
            "size");
    }
    ...
    for (uint32_t i = 0; i < 2; i++) {
      ...
      CHECK(
          sub[i].lumaSize().width < MIN_TB_SIZEY,
...
Common::Partitioning
Common::PartitionerImpl::getMaxTuTiling(const UnitArea &cuArea,
                                               const CodingStructure &cs) {
  ...
  const int maxTrSize =
      (area.width > 64 || area.height > 64) ? 64 : cs.sps->getMaxTbSize();
  ...
  CHECK(numTiles > MAX_CU_TILING_PARTITIONS,
        "CU partitioning requires more partitions than available");
...
void Common::PartitionerImpl::getTUIntraSubPartitions(
  ...
  if (splitType == TU_1D_HORZ_SPLIT) {
    ...
    for (uint32_t i = 0; i < nPartitions; i++) {
      ...
      blkY.y = i > 0 ? sub[i - 1].blocks[COMPONENT_Y].y + splitDimensionSize
                     : blkY.y;

      CHECK(sub[i].lumaSize().height < 1, "the cs split causes the block to be "
                                          "smaller than the minimal TU size");
    }
    ...
    for (uint32_t i = 0; i < nPartitions; i++) {
      ...
      blkY.x = i > 0 ? sub[i - 1].blocks[COMPONENT_Y].x + splitDimensionSize
                     : blkY.x;
      CHECK(
          sub[i].lumaSize().width < 1,
          "the split causes the block to be smaller than the minimal TU size");
    }
  } else {
    THROW("Unknown TU sub-partitioning");
  }
  // we only partition luma, so there is going to be only one chroma tu at the
  // end (unless it is dual tree, in which case there won't be any chroma
  // components)
  uint32_t partitionsWithoutChroma =
      (cs.area.chromaFormat == CHROMA_400)
          ? 0
          : (isDualTree ? nPartitions : nPartitions - 1);
...
Common::Partitioning
Common::PartitionerImpl::getSbtTuTiling(const UnitArea &cuArea,
                                               const CodingStructure &cs,
                                               const PartSplit splitType) {
  Partitioning ret;
  int numTiles = 2;
  int widthFactor, heightFactor, xOffsetFactor,
      yOffsetFactor; // y = (x * factor) >> 2;
  assert(splitType >= SBT_VER_HALF_POS0_SPLIT &&
         splitType <= SBT_HOR_QUAD_POS1_SPLIT);

  ret.resize(numTiles, cuArea);
  for (int i = 0; i < numTiles; i++) {
    if (splitType >= SBT_VER_QUAD_POS0_SPLIT) {
      if (splitType == SBT_HOR_QUAD_POS0_SPLIT ||
          splitType == SBT_HOR_QUAD_POS1_SPLIT) {
        widthFactor = 4;
        xOffsetFactor = 0;
        heightFactor = ((i == 0 && splitType == SBT_HOR_QUAD_POS0_SPLIT) ||
                        (i == 1 && splitType == SBT_HOR_QUAD_POS1_SPLIT))
                           ...
                           : 3;
        yOffsetFactor =
            (i == 0) ? 0 : (splitType == SBT_HOR_QUAD_POS0_SPLIT ? 1 : 3);
      } else {
        widthFactor = ((i == 0 && splitType == SBT_VER_QUAD_POS0_SPLIT) ||
                       (i == 1 && splitType == SBT_VER_QUAD_POS1_SPLIT))
                          ...
                          : 3;
        xOffsetFactor =
            (i == 0) ? 0 : (splitType == SBT_VER_QUAD_POS0_SPLIT ? 1 : 3);
        heightFactor = 4;
        yOffsetFactor = 0;
      }
    } else {
      if (splitType == SBT_HOR_HALF_POS0_SPLIT ||
          splitType == SBT_HOR_HALF_POS1_SPLIT) {
        widthFactor = 4;
        xOffsetFactor = 0;
        heightFactor = 2;
